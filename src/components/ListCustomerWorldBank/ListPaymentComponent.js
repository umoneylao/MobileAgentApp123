                            ...
                            <TouchableOpacity onPress={() => onDisplay(item)}>
                                <Animated.View style={{
                                    ...
                                    <View style={styles.nameContainer}>
                                        <View style={{ width: '60%' }}>
                                            {item.CustomerName ? <Text style={styles.nameTxt} numberOfLines={1} ellipsizeMode="tail">{item.CustomerName || null}</Text> : null}
                                            {item.PaymentCode ? <Text style={styles.nameTxt1} numberOfLines={1} ellipsizeMode="tail">{item.PaymentCode || null}</Text> : null}
                                            {item.customerName ? <Text style={styles.nameTxt1} numberOfLines={1} ellipsizeMode="tail">{item.customerName || null}</Text> : null}
                                            {item.cctId ? <Text style={styles.nameTxt1} numberOfLines={1} ellipsizeMode="tail">{item.cctId || null}</Text> : null}
                                            {item.status == 0 ?
                                                <Text style={styles.WaitLoad}>{I18n.t('WaitLoad')}</Text>
                                                : <Text style={styles.UploadedSuccessfully}>{I18n.t('UploadedSuccessfully')}</Text>}
                                            {/* <Text style={styles.msgTxt}>{item.status ? item.status == 0 ? I18n.t('WaitLoad') : I18n.t('UploadedSuccessfully') : null}</Text> */}
                                            <Text style={styles.msgTxt}>{item.payDate ? item.payDate : null}</Text>
                                        </View>
                                        <View style={{ width: '40%', height: '100%', }}>
                                            <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center', flex: 1 }}>
                                                {item.amount ? (
                                                    <Text style={styles.msgTxt1}>{item.amount ? item.amount : 0}</Text>
                                                ) : (
                                                    <Text style={styles.msgTxt1}>{item.Amount ? formatNumber(item.Amount.toString()) : 0}</Text>
                                                )}
