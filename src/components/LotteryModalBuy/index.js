...
export default class LotteryModal extends Component {
    ...
    onOpen(data) {
        this.setState({ visible: true, data, BuyLottery_id: data.BuyLottery_id, buyLottery: data.buyLottery, amountBuy: data.amountBuy })
    }
    ...
    onConfirm() {
        const { amountBuy, BuyLottery_id, buyLottery } = this.state
        this.props.onConfirm(BuyLottery_id, amountBuy, buyLottery)
        this.setState({ visible: false })
    }
    ...
    onChangeAmount = (text) => {
        ...
        this.setState({ amountBuy: formatNumber(text.trim()), errorAmount, selectedMoney: null })
    }
    render() {
        const { data, buyLottery, amountBuy, errorBuyLottery, errorAmount } = this.state
        return (
            <Modal
                                    ...
                                    <View style={styles.rowDetail}>
                                        ...
                                        <View style={styles.rowDetail}>
                                            <TextInput
                                                ...
                                                onSubmitEditing={() => {
                                                    this.setState({ errorBuyLottery: !buyLottery || errorBuyLottery ? I18n.t('errorEnterLottery') : null })
                                                }}
                                                icon={
                                                    !buyLottery ? null : !errorBuyLottery
                                    ...
                                    <View style={styles.rowDetail}>
                                        ...
                                        <View style={styles.rowDetail}>
                                            <TextInput
                                                ...
                                                onSubmitEditing={() => {
                                                    this.setState({ errorMoney: !amountBuy || errorAmount ? I18n.t('enterMoney') : null })
                                                }}
                                                icon={
                                                    !amountBuy ? null : !errorAmount
                                                        ? <Ionicons size={20} name='md-checkmark-circle' color={Colors.green} />
                                                        : <EvilIcons size={25} name='exclamation' color={Colors.fire} />
                                                }
                                            />
