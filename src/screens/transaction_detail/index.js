...
import {
  ...
  requestTranferBank,
  requestTranferBccs,
  reqCheckInfoDiscount,
} from "../../actions/Bank";
import {
  GET_OTP_BANK_FAILED,
  GET_OTP_BANK_SUCCESS,
  TRANSFER_EWALLET_TO_BANK_SUCCESS,
  TRANSFER_EWALLET_TO_BANK_FAILED,
} from "../../actions/types";
...
class TransactionDetail extends Component {
  constructor(props) {
    ...
    setTimeout(async () => {
      try {
        ...
        const pin = await AsyncStorage.getItem("pinFingerPay");
        const pinCode = await AsyncStorage.getItem("_getPin");

        const paybyFinger1 = await AsyncStorage.getItem("pinFingerPay");

        console.log("paybyFinger1",paybyFinger1)

        this.setState({
          // getPhone: onPhoneNumber,
          // getPin: onPinCode,
        //   pinFingerPay: pin,
          checkFinger: switchValue,
          pinFingerPay: pin ? pin : null
          // checkFingerPay:
        });
        console.log("====================================");
        console.log("checkFinger", this.state.pinFingerPay);
        console.log("====================================");
        console.log("====================================");
        console.log("checkFingerPin",pin);
        console.log("====================================");
      } catch (e) {
  ...
  onCheckDiscount(
    ...
    this.props.reqCheckInfoDiscount(
      carriedAccountId,
      fromAccountId,
      to_acc_id,
      processCode,
  ...
  onCodePin(text) {
    const errorCodePin =
      ...
      !isValidated(text, this.props.validatePhoneConst)
        ? I18n.t("pleaseInputYourCurrentPin")
        : null;
    this.setState({ codePin: text, errorCodePin });
  }
  async componentDidMount() {
    const pinFingerPay = await AsyncStorage.getItem("pinFingerPay");
    console.log("pinFingerPay123", pinFingerPay);

    const switchValue = await AsyncStorage.getItem("switchValue");
    if (switchValue !== null) {
      // checkFinger: switchValue == true ? true : false,
      this.setState({
        checkFinger: JSON.stringify(switchValue),
        pinFingerPay: JSON.stringify(pinFingerPay),
      });
      console.log("ComponentDidMount", this.state.checkFinger);
    }
  }
  loadSwitchValue = async () => {
    try {
      const switchValue = await AsyncStorage.getItem("switchValue");
      if (switchValue !== null) {
        this.setState({ checkFinger: JSON.parse(switchValue) });
      }
    } catch (error) {
      console.log(error);
    }
        ...
        ReactNativeBiometrics.simplePrompt({
          ...
          .then((resultObject) => {
            ...
            if (success) {
              ...
              const getPhone = infoAccount.phoneNumber;
              const getPin = this.state.pinFingerPay;
              this.onCheckLoginFingerpring(getPhone, getPin);
            }
          })
          .catch((err) => {
            console.log("Errorrrrrrrrr onFingerPrint", err);
          });
        // } else {
  ...
  onCheckLoginFingerpring = (getPhone, getPin) => {
    const { infoAccount } = this.props;
    console.log("====================================");
    console.log("onCheckLoginFingerpring");
    console.log("====================================");
    ...
    console.log("====================================");
    console.log(infoAccount, "infoAccountCheck");
    console.log("====================================");
  };
  onFingerpring = (getPin, infoAccount) => {
    this.setState({ codePin: getPin });
    if (this.state.codePin) {
      this.onPressProcess();
    }
    ...
    //     FIELD.PHONE_NUMBER
    //   );
    //   console.log("infoaccountCheckkkk", infoAccount);
    ...
    //   );
    //   RequestField.addToInitField(RequestField.addLanguage("en_US"));
    //   var data = RequestField.addToInitField(
    //     RequestField.addAccountID(infoAccount.accountId)
    //   );
    // let dataOTPBank = RequestField.addToInitField(
    ...
    // console.log("====================================");
    this.setState({ isRequestPinFringPrint: true });
    this.setState({
      // isLoading: true,
      setModalVisible: true,
      isRequestPinFringPrint: false,
      // byPassPIN: true,
      run: true,
      // isRequestOTP: true,
      // run: true,
  ...
  onPressProcess() {
    Keyboard.dismiss();
    const { infoAccount } = this.props;
    console.log("infoAccountCheck Onprocess");
    const {
      phone,
      money,
      message,
    ...
    switch (onProcess) {
      case "TRANSFER_NO_EWLLET":
        ...
        RequestField.addToInitField(
          RequestField.addServiceCode("GET_OTP_TRANSFER_NO_EWLLET")
        ); //42
        RequestField.addToInitField(RequestField.addActionNode("1")); //22
        let getOtpNonEwllet = RequestField.addToInitField(
          RequestField.addLanguage(infoAccount.language)
        ); //113
      ...
      case "TRANFER_EWALLET":
        if (byPassPIN && checkAmount) {
          RequestField.addToInitField(
            RequestField.addProcessCode(ConfigCode.MONEY_AGENT_CHARGE_CUSTOMER)
          );
          RequestField.addToInitField(
            RequestField.addRoleId(infoAccount.roleId)
          );
          RequestField.addToInitField(
            RequestField.addPhone(infoAccount.phoneNumber)
          );
          RequestField.addToInitField(RequestField.addPan(infoAccount.pan));
          RequestField.addToInitField(
            RequestField.addShopCode(infoAccount.agentCode)
          );
          RequestField.addToInitField(
            RequestField.addAccountID(infoAccount.accountId)
          );
          ...
          RequestField.addToInitField(RequestField.addPin(codePin)); //52
          RequestField.addToInitField(
            RequestField.addServiceCode("TRANFER_EWALLET")
          ); //41
          RequestField.addToInitField(RequestField.addActionNode("1")); //22
          let otpCashout = RequestField.addToInitField(
            RequestField.addLanguage(infoAccount.language)
          ); //113
          this.setState({ isRequestOTP: true, isLoading: true });
      ...
      case "MONEY_AGENT_TRANSFER_CUSTOMER":
        ...
        RequestField.addToInitField(RequestField.addPin(codePin)); //52
        RequestField.addToInitField(
          RequestField.addServiceCode("OTP_AGENT_TRANSFER_CUSTOMER")
        ); //42
        RequestField.addToInitField(RequestField.addActionNode("1")); //22
        let getOtpAgentTranferCus = RequestField.addToInitField(
          RequestField.addLanguage(infoAccount.language)
        ); //113
        this.setState({ isRequestOTP: true, isLoading: true });
      ...
      case "TRANSFER_E2E":
        if (byPassPIN && checkAmount) {
          ...
          if (toAccountId)
            RequestField.addToInitField(
              RequestField.addToAccountId(toAccountId)
            );
          if (toName)
            RequestField.addToInitField(RequestField.addToName(toName));
          if (toPhone)
          ...
          RequestField.addToInitField(
            RequestField.addTransactionDescription(message)
          );
          RequestField.addToInitField(RequestField.addTier(saveInfo));
          RequestField.addToInitField(RequestField.addTelex(1)); //27
          let dataTranfer = RequestField.addToInitField(
            RequestField.addActionNode("1")
          ); //22
          dataTranfer.fieldMap = _.orderBy(dataTranfer.fieldMap, "fieldID");
          this.setState({
            isRequestCashInForPOS: true,
            isLoading: true,
            setModalVisible: false,
          });
          ...
          RequestField.addToInitField(RequestField.addPin(codePin)); //52
          RequestField.addToInitField(
            RequestField.addServiceCode("TRANSFER_E2E")
          ); //41
          RequestField.addToInitField(RequestField.addActionNode("1")); //22
          let otpTransferPOS = RequestField.addToInitField(
            RequestField.addLanguage(infoAccount.language)
          ); //113
          this.setState({ isRequestOTP: true, isLoading: true });
      ...
      case "AGENT_TRANSFER_CASH_OUT":
        if (byPassPIN && checkAmount) {
          ...
          RequestField.addToInitField(RequestField.addTIN("1")); //68
          RequestField.addToInitField(RequestField.addTelex(1)); //27
          let dataAgentCashOut = RequestField.addToInitField(
            RequestField.addActionNode("1")
          ); //22
          dataAgentCashOut.fieldMap = _.orderBy(
            dataAgentCashOut.fieldMap,
            "fieldID"
          );
          this.setState({
            isAgentCashOut: true,
          ...
          RequestField.addToInitField(RequestField.addPin(codePin)); //52
          RequestField.addToInitField(
            RequestField.addServiceCode("AGENT_TRANSFER_CASH_OUT")
          ); //41
          RequestField.addToInitField(RequestField.addActionNode("1")); //22
          let otpCashOut = RequestField.addToInitField(
            RequestField.addLanguage(infoAccount.language)
      ...
      case "AGENT_REQUEST_BANK_MONEY":
        ...
        RequestField.addToInitField(RequestField.addPin(codePin)); //52
        RequestField.addToInitField(
          RequestField.addServiceCode("GET_OTP_REQUEST_BANK_MONEY")
        ); //42
        RequestField.addToInitField(RequestField.addActionNode("1")); //22
        let getOtpRequestBankMoney = RequestField.addToInitField(
          RequestField.addLanguage(infoAccount.language)
        ); //113
        console.log("getOtpRequestBankMoney:", getOtpRequestBankMoney);
        this.setState({ isRequestOTP: true, isLoading: true });
      ...
      case "AGENT_REQUEST_EMONEY":
        ...
        RequestField.addToInitField(RequestField.addPin(codePin)); //52
        RequestField.addToInitField(
          RequestField.addServiceCode("GET_OTP_REQUEST_MONEY")
        ); //42
        RequestField.addToInitField(RequestField.addActionNode("1")); //22
        let getOtpRequestMoney = RequestField.addToInitField(
          RequestField.addLanguage(infoAccount.language)
        ); //113
        this.setState({ isRequestOTP: true, isLoading: true });
      ...
      case "TRANSFER_EWALLET_TO_BANK":
        if (byPassPIN && checkAmount) {
          if (processCode === "600014") {
            RequestField.clearInitField();
            RequestField.addToInitField(
              RequestField.addProcessCode(processCode)
            ); //3
            RequestField.addToInitField(
              RequestField.addPhone(infoAccount.phoneNumber)
            ); //34
            ...
            RequestField.addToInitField(
              RequestField.addFromPhone(infoAccount.phoneNumber)
            ); //105
            RequestField.addToInitField(
              RequestField.addFromAccountId(infoAccount.accountId)
            ); //76
            // RequestField.addToInitField(RequestField.addPin(codePin))//52
            RequestField.addToInitField(RequestField.addPan(infoAccount.pan)); //2
            ...
            RequestField.addToInitField(
              RequestField.addBankAccountName(AccountBankName)
            ); //100
            RequestField.addToInitField(
              RequestField.addBankAccountNo(AccountNo)
            ); //88
            ...
            RequestField.addToInitField(RequestField.addTelex(1)); //27
            const dataTranferBank_BCEL = RequestField.addToInitField(
              RequestField.addLanguage(infoAccount.language)
            ); //113
            this.setState({
              isLoading: true,
              setModalVisible: false,
              isRequestTranferBank: true,
              setModalVisible: false,
            });
            ...
            RequestField.addToInitField(RequestField.addTelex(1)); //27
            const dataTranferBank = RequestField.addToInitField(
              RequestField.addLanguage(infoAccount.language)
            ); //113
          ...
          RequestField.addToInitField(RequestField.addActionNode("1")); //22
          let dataOTP = RequestField.addToInitField(
            RequestField.addLanguage(infoAccount.language)
          ); //113
          this.setState({ isRequestOTP: true, isLoading: true });
      ...
      case "TOP_UP_UNITEL":
        if (byPassPIN && checkAmount) {
          ...
          RequestField.clearInitField();
          RequestField.addToInitField(
            RequestField.addProcessCode(ConfigCode.PAYMENT_CHARGE_TELE_ACCOUNT)
          ); //571000
          RequestField.addToInitField(
            RequestField.addPhone(infoAccount.phoneNumber)
          );
          ...
          RequestField.addToInitField(RequestField.addToPhone(phone));
          RequestField.addToInitField(
            RequestField.addServiceIndicator(serviceIndicator)
          );
          RequestField.addToInitField(RequestField.addAmount(money));
          RequestField.addToInitField(RequestField.addPan(infoAccount.pan));
          RequestField.addToInitField(RequestField.addActionNode("1")); //22
          RequestField.addToInitField(RequestField.addTIN("1")); //68
          RequestField.addToInitField(
            RequestField.addServiceCode(serviceCodeUitel)
          );
          RequestField.addToInitField(
            RequestField.addPartnerCode(ConfigCode.VIETTEL)
          );
          RequestField.addToInitField(
            RequestField.addRoleId(infoAccount.roleId)
          );
          RequestField.addToInitField(RequestField.addTier(saveInfo)); //63
          RequestField.addToInitField(RequestField.addTelex(1)); //27
          let dataTopupUnitel = RequestField.addToInitField(
            RequestField.addAccountID(infoAccount.accountId)
          );
          ...
          RequestField.addToInitField(RequestField.addPin(codePin)); //52
          RequestField.addToInitField(
            RequestField.addServiceCode("TOP_UP_UNITEL")
          ); //41
          RequestField.addToInitField(
            RequestField.addServiceCode("TOP_UP_MOBILE")
          ); //42
          RequestField.addToInitField(RequestField.addActionNode("1")); //22
          let otpUnitel = RequestField.addToInitField(
            RequestField.addLanguage(infoAccount.language)
          ); //113
          this.setState({ isRequestOTP: true, isLoading: true });
      ...
      case "TOP_UP_ETL":
        if (byPassPIN && checkAmount) {
          RequestField.clearInitField();
          RequestField.addToInitField(
            RequestField.addProcessCode(ConfigCode.TRANSFER_TOPUP_ETL)
          ); //3
          RequestField.addToInitField(RequestField.addActionNode("1")); //22
          RequestField.addToInitField(
            RequestField.addPhone(infoAccount.phoneNumber)
          ...
          RequestField.addToInitField(RequestField.addTier(saveInfo)); //107
          RequestField.addToInitField(RequestField.addTelex(1)); //27
          const dataTopupETL = RequestField.addToInitField(
            RequestField.addLanguage(infoAccount.language)
          ); //113
          ...
          RequestField.addToInitField(RequestField.addActionNode("1")); //22
          let dataOTPETL = RequestField.addToInitField(
            RequestField.addLanguage(infoAccount.language)
          ); //113
          this.setState({ isRequestOTP: true, isLoading: true });
      ...
      case "INTERNET_SERVICES":
        ...
        RequestField.addToInitField(RequestField.addPin(codePin)); //52
        RequestField.addToInitField(
          RequestField.addServiceCode("INTERNET_SERVICES")
        ); //41
        RequestField.addToInitField(
          RequestField.addServiceCode("TOP_UP_MOBILE")
        ); //42
        RequestField.addToInitField(RequestField.addActionNode("1")); //22
        let otpInternet = RequestField.addToInitField(
          RequestField.addLanguage(infoAccount.language)
        ); //113
        this.setState({ isRequestOTP: true, isLoading: true });
      ...
      case "PAYMENT_PSTNH":
        ...
        RequestField.addToInitField(RequestField.addRoleId(infoAccount.roleId));
        let dataPSTN = RequestField.addToInitField(
          RequestField.addAccountID(infoAccount.accountId)
        );
      ...
      case "AGENT_REGISTER_FOR_USER":
        ...
        RequestField.addToInitField(RequestField.addPin(codePin)); //52
        RequestField.addToInitField(
          RequestField.addServiceCode("OTP_REGISTER_FOR_USER")
        ); //42
        RequestField.addToInitField(RequestField.addActionNode("1")); //22
        let getOtpRegister = RequestField.addToInitField(
          RequestField.addLanguage(infoAccount.language)
        ); //113
        this.setState({ isRequestOTP: true, isLoading: true });
      ...
      case "MONEY_POS_CASH_IN":
        if (byPassPIN && checkAmount) {
          RequestField.addToInitField(
            RequestField.addProcessCode(ConfigCode.MONEY_POS_CASH_IN)
          );
          RequestField.addToInitField(
            RequestField.addRoleId(infoAccount.roleId)
          );
          ...
          let dataCashinPOS = RequestField.addToInitField(
            RequestField.addShopCode(infoAccount.agentCode)
          );
          dataCashinPOS.fieldMap = _.orderBy(dataCashinPOS.fieldMap, "fieldID");
          this.setState({
            isRequestCashInPOS: true,
            isLoading: true,
            setModalVisible: false,
          });
          ...
          RequestField.addToInitField(RequestField.addPin(codePin)); //52
          RequestField.addToInitField(
            RequestField.addServiceCode("MONEY_POS_CASH_IN")
          ); //41
          RequestField.addToInitField(
            RequestField.addServiceCode("MONEY_POS_CASH_IN")
          ); //42
          RequestField.addToInitField(RequestField.addActionNode("1")); //22
          let otpPosCashIN = RequestField.addToInitField(
            RequestField.addLanguage(infoAccount.language)
          ); //113
          this.setState({ isRequestOTP: true, isLoading: true });
      ...
      case "TOP_UP_LTC":
        if (byPassPIN && checkAmount) {
          RequestField.clearInitField();
          RequestField.addToInitField(
            RequestField.addProcessCode(ConfigCode.TRANSFER_TOPUP_LTC)
          ); //3
          RequestField.addToInitField(RequestField.addActionNode("1")); //22
          RequestField.addToInitField(
            RequestField.addPhone(infoAccount.phoneNumber)
          ...
          RequestField.addToInitField(RequestField.addPaymentCode(phone)); //105
          RequestField.addToInitField(RequestField.addTier(saveInfo)); //107
          const dataTopupLTC = RequestField.addToInitField(
            RequestField.addLanguage(infoAccount.language)
          ); //113
          ...
          RequestField.addToInitField(RequestField.addActionNode("1")); //22
          let dataOTPLTC = RequestField.addToInitField(
            RequestField.addLanguage(infoAccount.language)
          ); //113
          this.setState({ isRequestOTP: true, isLoading: true });
      ...
      case "TELCO_TPLUS":
        if (byPassPIN && checkAmount) {
          RequestField.clearInitField();
          RequestField.addToInitField(
            RequestField.addProcessCode(ConfigCode.TOPUP_TPUST)
          ); //3
          RequestField.addToInitField(RequestField.addActionNode("1")); //22
          RequestField.addToInitField(
            RequestField.addPhone(infoAccount.phoneNumber)
          ...
          RequestField.addToInitField(RequestField.addTelex(1)); //27
          RequestField.addToInitField(
            RequestField.addPartnerCode("TELCO_TPLUS")
          ); //41
          RequestField.addToInitField(
            RequestField.addServiceCode("TOP_UP_MOBILE")
          ); //42
          RequestField.addToInitField(RequestField.addPaymentCode(phone)); //105
          RequestField.addToInitField(RequestField.addTier(saveInfo)); //107
          const dataTopupTpust = RequestField.addToInitField(
            RequestField.addLanguage(infoAccount.language)
          ); //113
          this.setState({
            isLoading: true,
          ...
          RequestField.addToInitField(RequestField.addPin(codePin)); //52
          RequestField.addToInitField(
            RequestField.addServiceCode("TELCO_TPLUS")
          ); //41
          RequestField.addToInitField(
            RequestField.addServiceCode("TOP_UP_MOBILE")
          ); //42
          RequestField.addToInitField(RequestField.addActionNode("1")); //22
          let dataOTPTPUST = RequestField.addToInitField(
            RequestField.addLanguage(infoAccount.language)
          ); //113
          this.setState({ isRequestOTP: true, isLoading: true });
      ...
      case "PAYMENT_LEASING":
        ...
        RequestField.addToInitField(RequestField.addPin(codePin)); //52
        RequestField.addToInitField(
          RequestField.addServiceCode("GET_OTP_LEASING")
        ); //42
        RequestField.addToInitField(RequestField.addActionNode("1")); //22
        let dataOTPLeasing = RequestField.addToInitField(
          RequestField.addLanguage(infoAccount.language)
        ); //113
        this.setState({ isRequestOTP: true, isLoading: true });
      ...
      case "BUY_LOTTERY":
        ...
        if (totalBuy <= 2000) {
          ...
          this.setState({
            responseMessage: I18n.t("amountMustbefrom", {
              amount: formatNumber(3000 + ""),
            }),
          });
        ...
        switch (support) {
          case "BUY_SOKXAY_LOTTERY":
            RequestField.clearInitField();
            RequestField.addToInitField(
              RequestField.addProcessCode(ConfigCode.BUY_LOTTERY)
            );
            RequestField.addToInitField(
              RequestField.addPhoneLottery(infoAccount.phoneNumber)
            );
            RequestField.addToInitField(
              RequestField.addToPhone(infoAccount.phoneNumber)
            );
            RequestField.addToInitField(
              RequestField.addFromPhone(infoAccount.phoneNumber)
            );
            ...
            RequestField.addToInitField(RequestField.addPan(infoAccount.pan));
            RequestField.addToInitField(
              RequestField.addImageName(newMiniState)
            );
            let dataBuyLottery = RequestField.addToInitField(
              RequestField.addAccountIDLottery(infoAccount.accountId)
            );
          ...
          case "BUY_NCC_LOTTERY":
            ...
            RequestField.addToInitField(RequestField.addPin(codePin)); //52
            RequestField.addToInitField(
              RequestField.addServiceCode("TELCO_ETL")
            ); //41
            RequestField.addToInitField(
              RequestField.addServiceCode("TOP_UP_MOBILE")
            ); //42
      ...
      case "SELL_LOTTERY":
        ...
        let newMiniStateSale = miniStateSale.substring(
          0,
          miniStateSale.length - 1
        );
        ...
        RequestField.addToInitField(RequestField.addPan(infoAccount.pan));
        RequestField.addToInitField(
          RequestField.addImageName(newMiniStateSale)
        );
        let dataSale = RequestField.addToInitField(
          RequestField.addAccountIDLottery(infoAccount.accountId)
        );
      ...
      case "CHECK_BACODE_LUCKY":
        ...
        RequestField.addToInitField(RequestField.addPin(codePin)); //52
        RequestField.addToInitField(
          RequestField.addServiceCode("BACODE_LUCKY")
        ); //41
        RequestField.addToInitField(RequestField.addActionNode("1")); //22
        let dataOTPLUCKY = RequestField.addToInitField(
          RequestField.addLanguage(infoAccount.language)
        ); //113
        this.setState({ isRequestOTP: true, isLoading: true });
      ...
      case "PAY_STAFF_DEBT":
        ...
        RequestField.addToInitField(RequestField.addPin(codePin)); //52
        RequestField.addToInitField(
          RequestField.addServiceCode("GET_OTP_BCCS")
        ); //42
        RequestField.addToInitField(RequestField.addActionNode("1")); //22
        let dataOTPBCCS = RequestField.addToInitField(
          RequestField.addLanguage(infoAccount.language)
        ); //113
        this.setState({ isRequestOTP: true, isLoading: true });
      ...
      case "APPROVAL_TRANSACTION":
        ...
        RequestField.addToInitField(RequestField.addPin(codePin)); //52
        RequestField.addToInitField(
          RequestField.addServiceCode("GET_OTP_APPROVAL_TRANSACTION")
        ); //42
        RequestField.addToInitField(RequestField.addActionNode("1")); //22
        let dataotpApproval = RequestField.addToInitField(
          RequestField.addLanguage(infoAccount.language)
        ); //113
        this.setState({ isRequestOTP: true, isLoading: true });
      ...
      case "PAY_MENT_INSURANCE_APA":
        ...
        RequestField.addToInitField(RequestField.addPin(codePin)); //52
        RequestField.addToInitField(
          RequestField.addServiceCode("GET_OTP_PAY_MENT_INSURANCE_APA")
        ); //42
        RequestField.addToInitField(RequestField.addActionNode("1")); //22
        let getOtpInsurance = RequestField.addToInitField(
          RequestField.addLanguage(infoAccount.language)
        ); //113
        this.setState({ isRequestOTP: true, isLoading: true });
      ...
      case "PAYMENT_WATTER_NPP":
        ...
        RequestField.addToInitField(RequestField.addPin(codePin)); //52
        RequestField.addToInitField(
          RequestField.addServiceCode("PAYMANT_WATTER")
        ); //41
        RequestField.addToInitField(RequestField.addActionNode("1")); //22
        let OTPWatter = RequestField.addToInitField(
          RequestField.addLanguage(infoAccount.language)
        ); //113
        this.setState({ isRequestOTP: true, isLoading: true });
      ...
      case "CASH_OUT_WORD_BANK":
        ...
        RequestField.addToInitField(RequestField.addPin(codePin)); //52
        RequestField.addToInitField(
          RequestField.addServiceCode("CASH_OUT_WORD_BANK")
        ); //41
        RequestField.addToInitField(RequestField.addActionNode("1")); //22
        let OTPWordBank = RequestField.addToInitField(
          RequestField.addLanguage(infoAccount.language)
        ); //113
        this.setState({ isRequestOTP: true, isLoading: true });
    ...
    switch (selectState) {
      ...
      case "LEASING_PAYMENT":
        if (text.length === 6) {
          ...
          RequestField.addToInitField(
            RequestField.addPartnerCode(partnerCodeLessing)
          ); //41
          ...
          RequestField.addToInitField(
            RequestField.addTransactionDescription("PAY_MENT_LEASING")
          ); //107
          RequestField.addToInitField(
            RequestField.addReceiverAddress(contractNo)
          ); //110
          RequestField.addToInitField(RequestField.addPan(infoAccount.pan));
          RequestField.addToInitField(
            RequestField.addLanguage(infoAccount.language)
          );
          let dataPayLeasing = RequestField.addToInitField(
            RequestField.addPin(codePin)
          ); //52
          dataPayLeasing.fieldMap = _.orderBy(
            dataPayLeasing.fieldMap,
            "fieldID"
          );
          this.setState({
            isRequestPaymentLeasing: true,
            isLoading: true,
            setModalVisible: false,
          });
      ...
      case "BUY_NCC_LOTTERY_NUMBER":
        if (text.length === 6) {
          ...
          if (onSelect === "lotteryNumber") {
            RequestField.addToInitField(
              RequestField.addProcessCode(ConfigCode.BUY_LOTTERY_NCC)
            );
          } else {
            RequestField.addToInitField(
              RequestField.addProcessCode(ConfigCode.BUY_LOTTERY_NCC_ANIMAL)
            );
          }
          ...
          RequestField.addToInitField(
            RequestField.addServiceCode("BUY_NCC_LOTTERY")
          ); //42
          RequestField.addToInitField(
            RequestField.addCurrencyCode(infoAccount.currencyCode)
          ); //49
          ...
          RequestField.addToInitField(RequestField.addPan(infoAccount.pan)); //2
          RequestField.addToInitField(
            RequestField.addToName(infoAccount.customerName)
          ); //98
          RequestField.addToInitField(
            RequestField.addCarriedName(infoAccount.customerName)
          ); //125
          let buyNccNumber = RequestField.addToInitField(
            RequestField.addAccountIDLottery(infoAccount.accountId)
          ); //78
      ...
      case "PAY_STAFF_DEBT":
        if (text.length === 6) {
          RequestField.clearInitField();
          RequestField.addToInitField(
            RequestField.addProcessCode(ConfigCode.PAY_STAFF_DEBT)
          ); //3
          RequestField.addToInitField(RequestField.addActionNode("1")); //22
          RequestField.addToInitField(
            RequestField.addPhone(infoAccount.phoneNumber)
          ...
          RequestField.addToInitField(
            RequestField.addPhonNumber(infoAccount.phoneNumber)
          ); //62
          RequestField.addToInitField(RequestField.addOPT(text)); //53
          RequestField.addToInitField(RequestField.addPan(infoAccount.pan)); //2
          RequestField.addToInitField(RequestField.addAmount(money)); //5
          ...
          RequestField.addToInitField(
            RequestField.addServiceCode("PAY_STAFF_DEBT")
          ); //42
          RequestField.addToInitField(
            RequestField.addFromPhone(infoAccount.phoneNumber)
          ); //105
          ...
          const dataTranferBccs = RequestField.addToInitField(
            RequestField.addLanguage(infoAccount.language)
          ); //113
      ...
      case "APPROVAL_TRANSACTION":
        if (text.length === 6) {
          RequestField.clearInitField();
          RequestField.addToInitField(
            RequestField.addProcessCode(ConfigCode.APPROVAL_TRANSACTION)
          ); //3
          RequestField.addToInitField(RequestField.addActionNode("1")); //22
          RequestField.addToInitField(
            RequestField.addPhoneLottery(infoAccount.phoneNumber)
          ...
          RequestField.addToInitField(
            RequestField.addEffectType(isChecked == true ? 0 : 1)
          ); //103
          ...
          const dataApproval = RequestField.addToInitField(
            RequestField.addLanguage(infoAccount.language)
          ); //113
      ...
      case "PAYMENT_BACODE_LUCKY":
        if (text.length === 6) {
          this.setState({
            isLoading: true,
            onCheckBacodeLottery: true,
            setModalVisible: false,
          });
          RequestField.clearInitField();
          RequestField.addToInitField(
            RequestField.addProcessCode(
              selectPartnert === "SOKXAY"
                ? ConfigCode.LUCKY_LOTTERY
                : selectPartnert === "NCC"
                ? ConfigCode.LUCKY_LOTTERY_NCC
                : selectPartnert === "NCC340"
                ? ConfigCode.LUCKY_LOTTERY_NCC340
                : null
            )
          ); //3
          ...
          let dataLucky = RequestField.addToInitField(
            RequestField.addAccountID(infoAccount.accountId)
          ); //78
      ...
      case "APA_INSURANCE":
        if (text.length === 6) {
          ...
          this.setState({
            isLoading: true,
            onChecPaymentInsurance: true,
            setModalVisible: false,
          });
          RequestField.clearInitField();
          RequestField.addToInitField(
            RequestField.addProcessCode(ConfigCode.PAY_MENT_INSURANCE_APA)
          ); //3
          RequestField.addToInitField(RequestField.addPan(infoAccount.pan)); //2
          ...
          RequestField.addToInitField(
            RequestField.addPartnerCode("APA_INSUARANCE")
          ); //41
          RequestField.addToInitField(RequestField.addServiceCode("ACCIDENT")); //42
          RequestField.addToInitField(
            RequestField.addCustomerName(InsuredName_APA)
          ); //46
          RequestField.addToInitField(
            RequestField.addCustomerBirthday(data[0] + data[1] + data[2])
          ); //47
          RequestField.addToInitField(
            RequestField.addCustomerGender(Gender_APA)
          ); //48
          RequestField.addToInitField(RequestField.addPin(codePin)); //52
          RequestField.addToInitField(RequestField.addOPT(text)); //53
          RequestField.addToInitField(RequestField.addPaperType(PaperType_APA)); //54
          RequestField.addToInitField(
            RequestField.addPaperNumber(PaperNumber_APA)
          ); //55
      ...
      case "PAYMENT_WATTER_NPP":
        if (text.length === 6) {
          this.setState({
            isLoading: true,
            PaymentWatterNpp: true,
            setModalVisible: false,
          });
          RequestField.clearInitField();
          RequestField.addToInitField(
            RequestField.addProcessCode(ConfigCode.PAYMANT_WATTER)
          ); //3
          RequestField.addToInitField(RequestField.addAmount(AmountBank)); //5
          RequestField.addToInitField(
            RequestField.addRoleId(infoAccount.roleId)
          ...
          RequestField.addToInitField(
            RequestField.addPartnerCode("PAYMENT_WATTER_NPP")
          ); //41
          RequestField.addToInitField(RequestField.addPin(codePin)); //52
          RequestField.addToInitField(RequestField.addPan(infoAccount.pan)); //2
          RequestField.addToInitField(RequestField.addOPT(text)); //53
      ...
      case "TRANFER_EWALLET":
        if (text.length === 6) {
          ...
          data.fieldMap = _.orderBy(data.fieldMap, "fieldID");
          this.setState({
            isRequestCashInForcustomer: true,
            isLoading: true,
            setModalVisible: false,
          });
          this.props.requestCashIn(data);
      ...
      case "INTERNET_SERVICES":
        if (text.length === 6) {
          ...
          RequestField.addToInitField(RequestField.addTIN("1")); //68
          RequestField.addToInitField(
            RequestField.addServiceCode(serviceCodeInternet)
          );
          RequestField.addToInitField(
            RequestField.addPartnerCode(ConfigCode.VIETTEL)
          );
          ...
          let dataInternetServices = RequestField.addToInitField(
            RequestField.addAccountID(infoAccount.accountId)
          );
          // console.log('dataInternetServices:', dataInternetServices)
          this.setState({
            isRequestPaymentInternet: true,
            isLoading: true,
            setModalVisible: false,
          });
      ...
      case "AGENT_REQUEST_BANK_MONEY":
        if (text.length === 6) {
          RequestField.clearInitField();
          RequestField.addToInitField(
            RequestField.addProcessCode(ConfigCode.AGENT_REQUEST_BANK_MONEY)
          ); // 033016
          RequestField.addToInitField(
            RequestField.addPhone(infoAccount.phoneNumber)
          );
          ...
          let dataRequestBank = RequestField.addToInitField(
            RequestField.addTransDes(message)
          );
          dataRequestBank.fieldMap = _.orderBy(
            dataRequestBank.fieldMap,
            "fieldID"
          );
          this.setState({
            isRequestBankMoney: true,
            isLoading: true,
            setModalVisible: false,
          });
      ...
      case "AGENT_REQUEST_EMONEY":
        if (text.length === 6) {
          RequestField.clearInitField();
          RequestField.addToInitField(
            RequestField.addProcessCode(ConfigCode.AGENT_REQUEST_EMONEY)
          ); // 030005
          RequestField.addToInitField(
            RequestField.addPhone(infoAccount.phoneNumber)
          );
          ...
          RequestField.addToInitField(RequestField.addTIN("1")); //68
          RequestField.addToInitField(RequestField.addOPT(text)); //53
          let dataAgentRequest = RequestField.addToInitField(
            RequestField.addTransDes(message)
          );
          dataAgentRequest.fieldMap = _.orderBy(
            dataAgentRequest.fieldMap,
            "fieldID"
          );
          this.setState({
            isRequestEMoney: true,
            isLoading: true,
            setModalVisible: false,
          });
      ...
      case "TRANSFER_NO_EWLLET":
        if (text.length === 6) {
          RequestField.clearInitField();
          RequestField.addToInitField(
            RequestField.addProcessCode(ConfigCode.TRANSFER_NE2NE)
          ); //011000
          RequestField.addToInitField(
            RequestField.addPhone(infoAccount.phoneNumber)
          );
          ...
          RequestField.addToInitField(RequestField.addOPT(text)); //53
          RequestField.addToInitField(
            RequestField.addTransDes(
              message ? message : "Transfer with U-money"
            )
          );
          let data1 = RequestField.addToInitField(
            RequestField.addCarriedAccId(infoAccount.accountId)
          );
          this.props.transferOtherToOther(data1);
          this.setState({
            isTransferOtherToOther: true,
            isLoading: true,
            setModalVisible: false,
          });
          RequestField.clearInitField();
      ...
      case "MONEY_AGENT_TRANSFER_CUSTOMER":
        if (text.length === 6) {
          ...
          RequestField.addToInitField(
            RequestField.addProcessCode(
              ConfigCode.MONEY_AGENT_TRANSFER_CUSTOMER
            )
          ); //010004
          ...
          RequestField.addToInitField(RequestField.addTIN("1")); //68
          RequestField.addToInitField(RequestField.addOPT(text)); //53
          let dataCashOut = RequestField.addToInitField(
            RequestField.addPin(codePin)
          );
          dataCashOut.fieldMap = _.orderBy(dataCashOut.fieldMap, "fieldID");
          this.setState({
            isRequestCashOut: true,
            isLoading: true,
            setModalVisible: false,
          });
      ...
      case "AGENT_REGISTER_FOR_USER":
        if (text.length === 6) {
          ...
          RequestField.addToInitField(
            RequestField.addProcessCode(ConfigCode.CUSTOMER_AGENT_REGISTER)
          ); //002001
          RequestField.addToInitField(
            RequestField.addPhone(infoAccount.phoneNumber)
          );
          ...
          RequestField.addToInitField(RequestField.addPan(infoAccount.pan));
          RequestField.addToInitField(
            RequestField.addCustomerPhone(phoneRegister)
          );
          RequestField.addToInitField(RequestField.addCustomerName(name));
          RequestField.addToInitField(
            RequestField.addCustomerBirthday(birthDay)
          );
          ...
          RequestField.addToInitField(
            RequestField.addAreaCode(ConfigCode.AREACODE)
          ); // docs is not require, but it must require
          RequestField.addToInitField(
            RequestField.addLanguage(infoAccount.language)
          );
          ...
          RequestField.addToInitField(RequestField.addTIN("1")); //68
          RequestField.addToInitField(RequestField.addOPT(text)); //53
          let dataRegister = RequestField.addToInitField(
            RequestField.addPaperNumber(ident)
          );
      ...
      case "CASH_OUT_WORD_BANK":
        if (text.length === 6) {
          RequestField.clearInitField();
          RequestField.addToInitField(
            RequestField.addProcessCode(ConfigCode.CASH_OUT_WORD_BANK)
          ); // 3
          RequestField.addToInitField(RequestField.addAmount(money)); //5
          ...
          RequestField.addToInitField(
            RequestField.addTransactionDescription(note)
          ); //107
          RequestField.addToInitField(
            RequestField.addPhone(infoAccount.phoneNumber)
          ); //34
          ...
          RequestField.addToInitField(
            RequestField.addEffectType(infoAccount.agentCode)
          ); //103
  ...
  ResendOTP() {
    ...
    switch (onProcess) {
      ...
      case "TRANSFER_EWALLET_TO_BANK":
        ...
        RequestField.addToInitField(RequestField.addPan(infoAccount.pan)); //2
        RequestField.addToInitField(RequestField.addPin(pinFingerPay)); //52
        RequestField.addToInitField(
          RequestField.addServiceCode("GET_OTP_BANK")
        ); //41
        RequestField.addToInitField(RequestField.addActionNode("1")); //22
        let dataOTPBank = RequestField.addToInitField(
          RequestField.addLanguage(infoAccount.language)
          ...
          noGuest[i].value != "0" ? (
            <View style={styles.groupText}>
              <Text style={styles.labelInfo}>
                {noGuest[i].typeOfPolicy == "1"
                  ? I18n.t("fee")
                  : noGuest[i].typeOfPolicy == "2"
                  ? I18n.t("commission")
                  : noGuest[i].typeOfPolicy == "3"
                  ? I18n.t("commission")
                  : noGuest[i].typeOfPolicy == "4"
                  ? I18n.t("discount")
                  : null}
              </Text>
  ...
  componentWillReceiveProps(nextProps) {
    ...
    var noGuest = this.state.dataItem;
    // const getpintest = 222222;
    // if (userFingerprint) {
    //   this.onFingerpring(pinFingerPay, infoAccount);
    //   this.setState({ userFingerprint: false });
    // } else {
    //   // this.onCheckBlockDeviceMobile(8562092616266);
    //   this.setState({
    ...
    //   });
    //   // this.savePhone(8562092616266);
    // }
    // if (isRequestPinFringPrint) {
        ...
        case "CHECK_INFO_DISCOUNT_SUCCESS":
          if (nextProps.dataDiscount) {
            let newDirectory = Object.values(
              nextProps.dataDiscount.reduce((acc, item) => {
                if (!acc[item.typeOfPolicy])
                  acc[item.typeOfPolicy] = {
      ...
      if (nextProps.cashInData) {
        ...
        switch (responseCode) {
          ...
          case responseCode:
            this.refs.responseMessage.onOpen();
            this.setState({
              isRequestCashInForPOS: false,
              isLoading: false,
              responseMessage: message ? message : I18n.t("99999"),
            });
            break;
          default:
            break;
    ...
    if (this.state.isTransferOtherToOther) {
      if (nextProps.transferOtO && nextProps.transferOtO.responseCode) {
        if (nextProps.transferOtO && nextProps.transferOtO.error === "00000") {
          let responseCode = RequestField.getValueField(
            nextProps.transferOtO.fieldMap,
            FIELD.RESPONSE_CODE
          );
          let message = RequestField.getValueField(
            nextProps.transferOtO.fieldMap,
    ...
    if (this.state.isRequestCashOut && !nextProps.isFetching) {
      ...
      if (nextProps.cashOutData && nextProps.cashOutData.responseCode) {
        if (nextProps.cashOutData && nextProps.cashOutData.error === "00000") {
          let responseCode = RequestField.getValueField(
            nextProps.cashOutData.fieldMap,
            FIELD.RESPONSE_CODE
          );
          let message = RequestField.getValueField(
            nextProps.cashOutData.fieldMap,
          ...
          switch (responseCode) {
            ...
            default:
              if (
                nextProps.cashOutData &&
                nextProps.cashOutData.responseDescription
              ) {
        ...
        if (nextProps.cashInData && nextProps.cashInData.error === "00000") {
          switch (nextProps.cashInData.responseCode) {
            case "00000":
              ...
              this.setState({
                isRequestCashInForcustomer: false,
                isLoading: false,
              });
              break;
            case nextProps.cashInData.responseCode:
              ...
              this.setState({
                isRequestCashInForcustomer: false,
                isLoading: false,
                responseMessage: nextProps.cashInData.responseDescription
                  ? nextProps.cashInData.responseDescription
                  : I18n.t("99998"),
              });
              break;
            default:
          ...
          if (nextProps.cashInData.responseCode) {
            ...
            this.setState({
              isRequestCashInForcustomer: false,
              isLoading: false,
              responseMessage: nextProps.cashInData.description
                ? nextProps.cashInData.description
                : I18n.t("99998"),
            });
          } else {
            this.refs.NotificationSystemBusy.onOpen();
    ...
    if (this.state.isAgentCashOut && !nextProps.isFetching) {
      ...
      if (nextProps.receiver && nextProps.receiver.responseCode) {
        if (
          nextProps.agentCashOutData &&
          nextProps.agentCashOutData.error === "00000"
        ) {
          let response = RequestField.getValueField(
            nextProps.agentCashOutData.fieldMap,
            FIELD.RESPONSE_CODE
          );
          let messenError = RequestField.getValueField(
            nextProps.agentCashOutData.fieldMap,
            FIELD.RESPONSE_DESCRIPTION
          );
          switch (response) {
            ...
            case response:
              this.refs.responseMessage.onOpen();
              this.setState({
                responseMessage: messenError ? messenError : I18n.t("99998"),
              });
              break;
            default:
              break;
          ...
          if (
            nextProps.agentCashOutData &&
            nextProps.agentCashOutData.responseCode
          ) {
    ...
    if (this.state.isRequestBankMoney && !nextProps.isFetching) {
      ...
      if (
        nextProps.agentReqBankMoneyData &&
        nextProps.agentReqBankMoneyData.responseCode
      ) {
        if (
          nextProps.agentReqBankMoneyData &&
          nextProps.agentReqBankMoneyData.error === "00000"
        ) {
          let response = RequestField.getValueField(
            nextProps.agentReqBankMoneyData.fieldMap,
            FIELD.RESPONSE_CODE
          );
          let messenError = RequestField.getValueField(
            nextProps.agentReqBankMoneyData.fieldMap,
    ...
    if (this.state.isRequestEMoney && !nextProps.isFetching) {
      ...
      if (
        nextProps.agentReqEMoneyData &&
        nextProps.agentReqEMoneyData.responseCode
      ) {
        if (
          nextProps.agentReqEMoneyData &&
          nextProps.agentReqEMoneyData.error === "00000"
        ) {
          let response = RequestField.getValueField(
            nextProps.agentReqEMoneyData.fieldMap,
            FIELD.RESPONSE_CODE
          );
          let messenError = RequestField.getValueField(
            nextProps.agentReqEMoneyData.fieldMap,
          ...
          switch (response) {
            ...
            case response:
              this.refs.responseMessage.onOpen();
              this.setState({
                responseMessage: messenError ? messenError : I18n.t("99999"),
              });
              break;
            default:
              if (response) {
                if (response === 10130) {
                  this.refs.responseMessage.onOpen();
                  this.setState({
                    responseMessage: I18n.t("bankCodeDoesNotExist", {
                      bank_code: bankCode,
                    }),
                  });
                } else {
                ...
                if (
                  nextProps.agentReqEMoneyData &&
                  nextProps.agentReqEMoneyData.responseDescription
                ) {
                  this.refs.transactionIsUnsuccessful.onOpen();
                } else {
                  this.refs.NotificationSystemBusy.onOpen();
    ...
    if (this.state.isRequestOTP && this.state.isLoading) {
      console.log(
        "nextProps.BankReducer.actionType:",
        nextProps.BankReducer.actionType
      );
      console.log(
        "nextProps.BankReducer.getOTPBank:",
        nextProps.BankReducer.getOTPBank
      );
      ...
      switch (nextProps.BankReducer.actionType) {
        case GET_OTP_BANK_SUCCESS:
          ...
          if (nextProps.BankReducer.getOTPBank.data) {
            if (
              nextProps.BankReducer.getOTPBank.data.responseCode === "00000"
            ) {
              this.setState({
                isLoading: false,
                setModalVisible: true,
      ...
      switch (nextProps.BankReducer.actionType) {
        case TRANSFER_EWALLET_TO_BANK_SUCCESS:
          if (nextProps.BankReducer.TransferToBank.data) {
            let response = RequestField.getValueField(
              nextProps.BankReducer.TransferToBank.data.fieldMap,
              FIELD.RESPONSE_CODE
            );
            let messenError = RequestField.getValueField(
              nextProps.BankReducer.TransferToBank.data.fieldMap,
    ...
    if (this.state.isRequestTopUp && !nextProps.isFetching) {
      ...
      if (nextProps.topUpData && nextProps.topUpData.responseCode) {
        ...
        switch (response) {
          ...
          default:
            var responseCode =
              nextProps.topUpData && nextProps.topUpData.responseCode
                ? nextProps.topUpData.responseCode
                : "10532";
            if (responseCode) {
    ...
    if (this.state.isRequestTopUpETL && !nextProps.isFetching) {
      ...
      if (nextProps.requestTopUpETL.data.error === "00000") {
        let response = RequestField.getValueField(
          nextProps.requestTopUpETL.data.fieldMap,
          FIELD.RESPONSE_CODE
        );
        let messenError = RequestField.getValueField(
          nextProps.requestTopUpETL.data.fieldMap,
    ...
    if (this.state.isRequestTopUpPSTN && !nextProps.isFetching) {
      ...
      if (nextProps.topUpData && nextProps.topUpData.responseCode) {
        if (
          nextProps.topUpData &&
          nextProps.topUpData.responseCode === "00000"
        ) {
          this.props.navigation.navigate("TransactionResult", {
            data: nextProps.topUpData.fieldMap,
          ...
          if (responseCode) {
            ...
            if (
              nextProps.topUpData &&
              nextProps.topUpData.responseDescription
            ) {
              this.refs.transactionIsUnsuccessful.onOpen();
            } else {
              this.refs.NotificationSystemBusy.onOpen();
    ...
    if (this.state.isRegister && !nextProps.isRegFetching) {
      ...
      if (
        nextProps.agentRegForUserData &&
        nextProps.agentRegForUserData.responseCode
      ) {
        if (nextProps.agentRegForUserData.error === "00000") {
          let response = RequestField.getValueField(
            nextProps.agentRegForUserData.fieldMap,
            FIELD.RESPONSE_CODE
          );
          let messenError = RequestField.getValueField(
            nextProps.agentRegForUserData.fieldMap,
    ...
    if (this.state.isLoading && this.state.isRequestPaymentLeasing) {
      ...
      switch (nextProps.LeasingReducer.actionType) {
        case "PAY_LEASING_AEON_SUCCESS":
          let messenError = RequestField.getValueField(
            nextProps.requestPayLeasingAeon.fieldMap,
            FIELD.RESPONSE_DESCRIPTION
          );
            ...
            switch (nextProps.LeasingReducer.responseCode) {
              case nextProps.LeasingReducer.responseCode:
                ...
                this.setState({
                  isRequestPaymentLeasing: false,
                  isLoading: false,
                  responseMessage: nextProps.LeasingReducer.responseDescription,
                });
                break;
    ...
    if (this.state.onBuyLottery && this.state.isLoading) {
      if (
        nextProps.LotteryReducer.isSuccess === true &&
        nextProps.LotteryReducer
      ) {
        this.setState({ isLoading: false, onBuyLottery: false });
        let response = RequestField.getValueField(
          nextProps.LotteryReducer.buyLottery.data.fieldMap,
          FIELD.RESPONSE_CODE
        );
        let messenError = RequestField.getValueField(
          nextProps.LotteryReducer.buyLottery.data.fieldMap,
          ...
          case "BUY_LOTTERY_SUCCESS":
            switch (response) {
              case "00000":
                ...
                let list = RequestField.getValueField(
                  nextProps.LotteryReducer.buyLottery.data.fieldMap,
                  FIELD.FILE_NAME
                );
                if (responseCode === "00000") {
              ...
              case 10000:
                ...
                this.setState({
                  responseMessage:
                    nextProps.LotteryReducer.buyLottery.data
                      .responseDescription,
                });
                break;
              case response:
      ...
      if (
        ...
        switch (nextProps.LotteryReducer.actionType) {
          case "SALE_LOTTERY_SUCCESS":
            if (
              nextProps.LotteryReducer.saleLottery.data.error === "00000" &&
              nextProps.LotteryReducer.saleLottery.data.responseCode === "00000"
            ) {
              let responseCode = RequestField.getValueField(
                nextProps.LotteryReducer.saleLottery.data.fieldMap,
                FIELD.RESPONSE_CODE
              );
              let list = RequestField.getValueField(
                nextProps.LotteryReducer.saleLottery.data.fieldMap,
        ...
        switch (nextProps.LotteryReducer.actionType) {
          case "LUCKY_LOTTERY_SUCCESS":
            if (nextProps.LotteryReducer.luckyLottery.data.error === "00000") {
              let responseLucky = RequestField.getValueField(
                nextProps.LotteryReducer.luckyLottery.data.fieldMap,
                FIELD.RESPONSE_CODE
              );
              let meassge = RequestField.getValueField(
                nextProps.LotteryReducer.luckyLottery.data.fieldMap,
                FIELD.RESPONSE_DESCRIPTION
              );
              switch (responseLucky) {
                ...
                case responseLucky:
                  this.refs.responseMessage.onOpen();
                  this.setState({
                    responseMessage: meassge ? meassge : I18n.t("99999"),
                  });
                  break;
      ...
      switch (nextProps.actionType) {
        case "PAY_STAFF_DEBT_SUCCESS":
          if (nextProps.transferbccs.data) {
            let responseLucky = RequestField.getValueField(
              nextProps.transferbccs.data.fieldMap,
              FIELD.RESPONSE_CODE
            );
            let meassge = RequestField.getValueField(
              nextProps.transferbccs.data.fieldMap,
    ...
    if (this.state.isRequestApproval && this.state.isLoading) {
      switch (nextProps.WorldBank.actionType) {
        case "APPROVAL_TRANSACTION_SUCCESS":
          let responseCode = RequestField.getValueField(
            nextProps.WorldBank.getDataApproval.data.fieldMap,
            FIELD.RESPONSE_CODE
          );
          let responseDescripTion = RequestField.getValueField(
            nextProps.WorldBank.getDataApproval.data.fieldMap,
            FIELD.RESPONSE_DESCRIPTION
          );
          switch (responseCode) {
            ...
            case responseCode:
              ...
              this.setState({
                ...
                isLoading: false,
                messageError: responseDescripTion,
              });
              break;
            default:
              break;
      ...
      if (nextProps.dataPayMentInsurance && nextProps.Insurance.actionType) {
        switch (nextProps.Insurance.actionType) {
          case "PAY_MENT_INSURANCE_SUCCESS":
            let responseCode = RequestField.getValueField(
              nextProps.dataPayMentInsurance.data.fieldMap,
              FIELD.RESPONSE_CODE
            );
            let responseDescripTion = RequestField.getValueField(
              nextProps.dataPayMentInsurance.data.fieldMap,
          ...
          case "PAY_MENT_INSURANCE_FAILED":
            ...
            this.setState({
              onChecPaymentInsurance: false,
              isLoading: false,
              messageError: "PAY_MENT_INSURANCE_FAILED",
            });
            break;
      ...
      if (nextProps.getPaymentWatter && nextProps.WatterReducers.actionType) {
        ...
        switch (nextProps.WatterReducers.actionType) {
          case "PAY_MENT_WATTER_NPP_SUCCESS":
            let responseCode = RequestField.getValueField(
              nextProps.getPaymentWatter.data.fieldMap,
              FIELD.RESPONSE_CODE
            );
            let responseDescripTion = RequestField.getValueField(
              nextProps.getPaymentWatter.data.fieldMap,
          ...
          case "PAY_MENT_WATTER_NPP_FAILED":
            ...
            this.setState({
              PaymentWatterNpp: false,
              isLoading: false,
              messageError: "PAY_MENT_WATTER_NPP_FAILED",
            });
            break;
          default:
            break;
    ...
    if (this.state.isRequestCashOutWordBank && this.state.isLoading) {
      console.log(
        "nextProps:-actionTypeWordBank---",
        nextProps.actionTypeWordBank
      );
      console.log(
        "nextProps.cashOutWordBankinfo:-----",
        nextProps.cashOutWordBankinfo
      );
      switch (nextProps.actionTypeWordBank) {
        case "CASH_OUT_WORD_BANK_SUCCESS":
          let response = RequestField.getValueField(
            nextProps.cashOutWordBankinfo.fieldMap,
            FIELD.RESPONSE_CODE
          );
          let description = RequestField.getValueField(
            nextProps.cashOutWordBankinfo.fieldMap,
            FIELD.RESPONSE_DESCRIPTION
          );
          ...
          switch (response) {
            case "00000":
              ...
              this.setState({
                isLoading: false,
                isRequestCashOutWordBank: false,
              });
              break;
            case 11101:
              this.refs.responseDescripTion.onOpen();
  ...
  somethingWentWrong() {
    this.refs.somethingWentWrong.onClose();
  }
  Error11101() {
    this.refs.Error11101.onClose();
  }
  Error10157() {
    this.refs.Error10157.onClose();
  }
  error10155() {
    this.refs.error10155.onClose();
  }
  error10157() {
    this.refs.error10157.onClose();
  }
            ...
            <View style={{ flexDirection: "row", width: "100%" }}>
              <View style={{ padding: 20, width: "50%" }}>
                ...
                {getFee == "0" ? (
                  <View style={styles.groupText}>
                    ...
                    <Text style={styles.valueFree} numberOfLines={2}>
                      {I18n.t("free")}
                    </Text>
                  </View>
                ) : null}
              ...
              <View style={{ padding: 20, width: "50%" }}>
                  ...
                  <View style={styles.groupText}>
                    ...
                    <Text style={styles.valueInfo} numberOfLines={2}>
                      {id_Bank === "BANK_LVB"
                        ? I18n.t("LaoVietBank")
                        : id_Bank === "BANK_BCEL"
                        ? I18n.t("BCEL_fullName")
                        : id_Bank === "BANK_MARUHAN"
                        ? I18n.t("Maruhan_fullName")
                        : id_Bank === "BANK_LDB"
                        ? I18n.t("LDB_fullName")
                        : id_Bank === "BANK_ACLEDA"
                        ? I18n.t("Acleda_fullName")
                        : id_Bank === "BANK_SACOM"
                        ? I18n.t("SACOM_fullname")
                        : id_Bank === "BANK_BIC"
                        ? I18n.t("BIC_fullname")
                        : id_Bank === "BANK_APB"
                        ? I18n.t("APB_fullname")
                ...
                {newMiniState ? (
                  <View style={styles.groupText}>
                    <Text style={styles.labelInfo}>
                      {I18n.t("numberLottery")}
                    </Text>
                    <Text style={styles.textValue} numberOfLines={6}>
                      {newMiniState}
                    </Text>
                  </View>
                ) : null}
                ...
                {onProcess === "TRANSFER_E2E" ? null : phone ? (
                  <View style={styles.groupText}>
                    <Text style={styles.labelInfo}>
                      {I18n.t("receiverPhone")}
                    </Text>
                ...
                {GRAND_TOTAL_APA ? (
                  <View style={styles.groupText}>
                    <Text style={styles.labelInfo}>
                      {I18n.t("ValueTaxIncluded")}
                    </Text>
                    <Text style={styles.valueInfo} numberOfLines={2}>
                      {formatNumber(GRAND_TOTAL_APA)} ₭
                    </Text>
                  </View>
                ) : null}
                {totalAmont ? (
              ...
              {checkFinger=='true' ? (
                <View style={styles.fingerPrintContainer}>
                  <TouchableOpacity onPress={() => this.onFingerPrint()} >
                    <View style={styles.fingerAndText} >
                    <Image
                      source={Platform.OS=='ios' ? Images.ic_Faceid : Images.ic_FingerPrint}
                      style={styles.fingerPrintIcon}
                    />
                    <Text style={styles.titleInfo}>{Platform.OS=='ios' ? I18n.t("faceIdVerification"): I18n.t("fingerprintVeri")}</Text>
                    </View>
                  </TouchableOpacity>
                </View>
              ) : null}
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    requestCashIn: (data) => {
      dispatch(requestCashIn(data));
    },
    transferOtherToOther: (info) => {
      dispatch(transferOtherToOther(info));
    },
    requestCashOut: (data) => {
      dispatch(requestCashOut(data));
    },
    requestCashInTranfer: (data) => {
      dispatch(requestCashInTranfer(data));
    },
    agentReqBankMoney: (data) => {
      dispatch(agentReqBankMoney(data));
    },
    agentRequestEmoney: (data) => {
      dispatch(agentRequestEmoney(data));
    },
    requestOTP: (data) => {
      dispatch(requestOTP(data));
    },
    requestTranferBank: (data) => {
      dispatch(requestTranferBank(data));
    },
    requestTopUp: (data) => {
      dispatch(requestTopUp(data));
    },
    requestTopup_ETL: (data) => {
      dispatch(requestTopup_ETL(data));
    },
    agentRegForUser: (data) => {
      dispatch(agentRegForUser(data));
    },
    paymentLeasingAeon: (data) => {
      dispatch(paymentLeasingAeon(data));
    },
    requestBuyLottery: (data) => {
      dispatch(requestBuyLottery(data));
    },
    requestBuyLotteryNCC: (data) => {
      dispatch(requestBuyLotteryNCC(data));
    },
    requestSaleLottery: (data) => {
      dispatch(requestSaleLottery(data));
    },
    luckyLottery: (data) => {
      dispatch(luckyLottery(data));
    },
    requestTranferBccs: (data) => {
      dispatch(requestTranferBccs(data));
    },
    requestApproval: (data) => {
      dispatch(requestApproval(data));
    },
    paymentInsurance: (data) => {
      dispatch(paymentInsurance(data));
    },
    PaymentWatterNpp: (data) => {
      dispatch(PaymentWatterNpp(data));
    },
    onCashOutWordBank: (data) => {
      dispatch(onCashOutWordBank(data));
    },
    reqCheckInfoDiscount: (
      carriedAccountId,
      fromAccountId,
